#############################################################
# Modelos de ocupación "single-season: paquete unmarked"
# Capítulo 10 libro Kéry y Royle (2016) Pp. 590-600
# SMandujanoR
# Algunos cambios al Script original: Sep 8, 2020
# Pueden seguir paso a paso este ejemplo observando el HTML
#############################################################
...
round(cbind(fm1@AIC,fm2@AIC,fm3@AIC,fm4@AIC,fm5@AIC,fm6@AIC,fm7@AIC,fm8@AIC,fm9@AIC,fm10@AIC,fm11@AIC,fm12@AIC),1) # mejor modelo 10
...
fms1 <- fitList("p(date+dur+I(dur^2)) psi(ele) " = fm13,
                "p(date+dur+I(dur^2) psi(ele+ele2) " = fm14,
                "p(date+dur+I(dur^2) psi(ele+ele2+ele3)" = fm15)
(ms <- modSel(fms1))

cbind(fm13@AIC, fm14@AIC, fm15@AIC) # mejor modelo 14

# Checando el efecto del "forest"
...
fms2 <- fitList("psi(ele+forest) p(date+dur)" = fm16,
                "psi(elev+forest+forest2) p(date+dur)" = fm17,
                "psi(elev+forest+forest2+elev:forest) p(date+dur)" = fm18,
                "psi(elev+forest+forest2+elev:forest+elev:forest2) p(date+dur)" = fm19,
                "psi(elev + forest + forest2 + elev:forest + elev:forest2 + elev2:forest) p(date + dur)" = fm20)

(ms <- modSel(fms2)) # 

cbind(fm16@AIC,fm17@AIC,fm18@AIC,fm19@AIC,fm20@AIC) # mejor modelo 20

# -----------------------------
# OJO: para ver coeficientes 
...
# PASO 4: GoF empleando el modelo fm20
# (ME LO SALTO POR EL MOMENTO PUES CONSUME MUCHO TIEMPO)
##library(AICcmodavg)
###system.time(gof.boot <- mb.gof.test(fm20, nsim = 100))
###gof.boot

# -------------------------
# PASO 5: para graficar 

# se crean primero nuevas covariables
...
plot(pred.occ.elev[[1]]~orig.elev, type="l", lwd=3, col="blue", ylim=c(0,1), las=1, ylab="Pred.occ.prob.", xlab="Elevation (m)", frame=F)
matlines(orig.elev, pred.occ.elev[,3:4], lty=1, lwd=1, col="grey")

plot(pred.occ.forest[[1]]~orig.forest, type= "l", lwd=3, col="blue", ylim=c(0,1), las=1, ylab="Pred.occ.prob.", xlab="Forest cover (%)", frame=F)
matlines(orig.forest, pred.occ.forest[,3:4], lty=1, lwd=1, col="grey")

plot(pred.det.date[[1]]~orig.date, type="l", lwd=3, col="blue", ylim=c(0,1), las=1, ylab="Pred.det.prob.", xlab="Date (1= 1 April)", frame=F)
matlines(orig.date, pred.det.date[,3:4], lty=1, lwd=1, col="grey")

plot(pred.det.dur[[1]]~orig.duration, type="l", lwd=3, col="blue", ylim=c(0,1), las=1, ylab="Pred.det.prob.", xlab="Survey duration (min)", frame=F)
matlines(orig.duration, pred.det.dur[,3:4], lty=1, lwd=1, col="grey")

# predicción por covariables simultáneas

pred.matrix1 <- pred.matrix2 <- array(NA, dim= c(100,100)) 
for(i in 1:100){
  for(j in 1:100){
    newData1 <- data.frame(elev=ep[i], forest=fp[j]) # ocupación
    pred <- predict(fm20, type="state", newdata=newData1)
    pred.matrix1[i, j] <- pred$Predicted
    newData2 <- data.frame(dur=durp[i], date=dp[j]) # detección
...
box()
title(main="Probabilidad ocupación esperada", font.main=1)
...
axis(2, at= seq(0,100, by=10))
box()
title(main="Probabilidad detección esperada", font.main=1)
